...
int telitParam_setATEcho ( char* data )
{
    ...
    COMD_AT_PROCESS ( "ATE", 3000, at_rep, sizeof ( at_rep ) );
...
int telitParam_updatePDPreturnFormat ( void )
{
    ...
    {
        COMD_AT_PROCESS ( "AT+CGPIAF=1", 3000, at_rep, sizeof ( at_rep ) );
    }

    return 0;
}

int telitParam_setCfunMode( char* mode )
{
    char cfunSet_ret[4] = {0};

    telitParam_radioOnOffSet ( mode );
...
int telitParam_updateModuleVersion ( char* data )
{
    ...
    while ( check_times++ < 5 )
    {
        ...
        {
            if ( 0 != parsing_telit_moduleModel ( at_rep ) )
            {
                continue;
            }
        }
        ...
        {
            if ( 0 != parsing_telit_moduleVersion ( at_rep ) )
            {
                continue;
            }
        }
    ...
    //Jsut execute qmicli to get anything once for init qmicli-wds. Ignore if it exec fail or success
    system("telit-CM.sh 1 init");
    return 0; // do not return -1, even can not get module info
}

int telitParam_updateImei ( char* data )
{
    unsigned int check_times = 0;
    char imei_val[32] = {0};
...
int telitParam_disableSimslotHotplug(void)
{
    char at_rep[STR_AT_RSP_LEN] = {0};

    COMD_AT_PROCESS ( "AT#SIMINCFG?", 1500, at_rep, sizeof ( at_rep ) );
    if (    strstr ( at_rep, "\r\nOK\r\n" ) )
    {
        COMD_AT_PROCESS ( "AT#SIMINCFG=1,1", 1500, at_rep, sizeof ( at_rep ) );
        COMD_AT_PROCESS ( "AT#SIMINCFG=2,1", 1500, at_rep, sizeof ( at_rep ) );
    }
...
int telitParam_updateSuppBands ( char* data )
{
    ...
    COMD_AT_PROCESS ( "AT#BND=?", 3000, at_rep, sizeof ( at_rep ) );
    parsing_telit_bandlist(at_rep, lte_bands, sizeof(lte_bands), nr5g_bands, sizeof(nr5g_bands));
    ...
    nv_set("arfcnlock_type", TELIT_LOCK_EARFCN_MAX);
    nv_set("pcilock_type", TELIT_LOCK_PCI_MAX);

    return 0;
}

int telitParam_initC5greg( char* data )
{
    char at_rep[64] = {0};

    if( at_dev.comd_mVendor == M_VENDOR_TELIT_5G )
    {
        COMD_AT_PROCESS ( "AT+C5GREG=2", 3000, at_rep, sizeof ( at_rep ) );
...
int telitParam_initAPNsetting( char* data )
{
    ...
    if( data )
    {
        ...
        snprintf ( strRamOpt, sizeof ( strRamOpt ), "/usr/sbin/telit-CM.sh %d stop", apnset_start_index );
        CLOGD( FINE, "Execute [%s]", strRamOpt);
        system(strRamOpt);
        comd_wait_ms ( 1500 );
    ...
    for ( i = apnset_start_index; i <= apnset_end_index; i++ )
    {
        ...
        {
            snprintf ( at_cmd, sizeof ( at_cmd ), "AT#PDPAUTH=%d,0", i );
        }
        else
        {
            snprintf ( at_cmd, sizeof ( at_cmd ), "AT#PDPAUTH=%d,%s,\"%s\",\"%s\"",
                                                        i, uci_authtype, uci_username, uci_password );
        }
        memset ( at_rep, 0, sizeof ( at_rep ) );
        COMD_AT_PROCESS ( at_cmd, 3000, at_rep, sizeof ( at_rep ) );
...
static int telitParam_init_apn_netdev (void)
{
    nv_set ( "apn1_netdev", "qmimux0" );
    nv_set ( "apn2_netdev", "qmimux1" );
    nv_set ( "apn3_netdev", "qmimux2" );
    nv_set ( "apn4_netdev", "qmimux3" );
...
int telitParam_radioLockInit( char* data )
{
    ...
    if(lock_init_flag == 0)
    {
        ...
        sys_get_config ( "lte_param.scanmode.locked_band5g", uciLocked5gBand, sizeof ( uciLocked5gBand ) );
        sys_get_config ( "lte_param.scanmode.locked_pci", uciLockedPci, sizeof ( uciLockedPci ) );
        sys_get_config ( "lte_param.scanmode.locked_freq", uciLockedFreq, sizeof ( uciLockedFreq ) );
        ...
        if ( 0 == strcmp ( uciScanMode, "BAND_LOCK" ) )
        {
            ...
            telitParam_lockband ( uciLockedBand, 1 );
            nv_get ( "lockband_status", nvmra_buf, sizeof ( nvmra_buf ) );

            if(strcmp(nvmra_buf,"locked") == 0)
            {
                lock_init_flag = 1;
            }

        }
        else if ( 0 == strcmp ( uciScanMode, "FERQ_LOCK" ) && 0 != strcmp ( uciLockedFreq, "" ))
        {

            CLOGD ( FINE, "locked_freq -> [%s]\n", uciLockedFreq );

            comd_strrpl ( uciLockedFreq, " ", "_" );
            telitParam_lockearfcn ( uciLockedFreq );
            nv_get ( "lockearfcn_status", nvmra_buf, sizeof ( nvmra_buf ) );
        ...
        else if ( 0 == strcmp ( uciScanMode, "PCI_LOCK" ) && 0 != strcmp ( uciLockedPci, "" ))
        {
            comd_strrpl ( uciLockedPci, " ", "_" );
            telitParam_lockpci ( uciLockedPci );
            nv_get ( "lockpci_status", nvmra_buf, sizeof ( nvmra_buf ) );
...
void telitParam_updateImsi (void)
{
    ...
    if ( 0 == strcmp ( ram_val, "" ) || 0 == strcmp ( ram_val, "--" ) )
    {
        ...
        if ( strcmp ( at_rep, "" ) )
            parsing_telit_cimi ( at_rep );

        telitParam_updateUsimMncLen();
    }
}
...
void telitParam_updateUsimMncLen (void)
{
    ...
    if ( strstr ( at_rep, "+CRSM:" ) )
        parsing_telit_simMncLen ( at_rep );
}
...
void telitParam_updateCgatt (void)
{
    ...
    if ( strcmp ( at_rep, "" ) )
        parsing_telit_cgatt ( at_rep );
}

void teliParam_updateServingCell ( char* data )
{
    char at_rep[RECV_BUF_SIZE] = {0};
    ...
    COMD_AT_PROCESS ( "AT#LTEDS", 3000, at_rep, sizeof ( at_rep ) );

    parsing_telit_serving_4gcellinfo ( at_rep );

    if( at_dev.comd_mVendor == M_VENDOR_TELIT_5G )
    {
        memset(at_rep, 0 ,sizeof(at_rep));
        COMD_AT_PROCESS ( "AT#NRDS", 3000, at_rep, sizeof ( at_rep ) );

        ret = parsing_telit_serving_5gcellinfo_adv ( at_rep );

        if(ret == 0)
        {
            COMD_AT_PROCESS ( "AT#RFSTS", 3000, at_rep, sizeof ( at_rep ) );

            ret = parsing_telit_serving_5gcellinfo_basic ( at_rep );

            memset(at_rep, 0 ,sizeof(at_rep));
            COMD_AT_PROCESS ( "AT#5GLINKSTAT?", 3000, at_rep, sizeof ( at_rep ) );

            parsing_telit_serving_5g_linkstat ( at_rep );
...
void teliParam_updateNeighbourCell(void)
{
    char at_rep[RECV_BUF_SIZE] = {0};
    int ret = 0;
    COMD_AT_PROCESS ( "AT#MONI=1", 3000, at_rep, sizeof ( at_rep ) );
    if(strstr(at_rep,"OK"))
    {
        memset(at_rep,0,sizeof(at_rep));
        COMD_AT_PROCESS ( "AT#MONI", 3000, at_rep, sizeof ( at_rep ) );
        if(strstr(at_rep,"OK"))
        {
            parsing_telit_neighbour_cell(at_rep, "intra", &ret);
        }
    }

    COMD_AT_PROCESS ( "AT#MONI=2", 3000, at_rep, sizeof ( at_rep ) );
    if(strstr(at_rep,"OK"))
    {
        memset(at_rep,0,sizeof(at_rep));
        COMD_AT_PROCESS ( "AT#MONI", 3000, at_rep, sizeof ( at_rep ) );
        if(strstr(at_rep,"OK"))
        {
            parsing_telit_neighbour_cell(at_rep, "inter", &ret);
        }
    }
...
void telitParam_updateC5greg (void)
{
    ...
    if( at_dev.comd_mVendor == M_VENDOR_TELIT_5G )
    {
        ...
        if ( strcmp ( at_rep, "" ) )
            parsing_telti_c5greg(at_rep);
    }
}

static void telitParam_updateSignalBar (void)
{
    char signal_s[8] = {0};
    int signal_i = 0;
    char ram_val[8] = {0};
...
int telitParam_configCgact ( int cid_index, int act_val )
{
    ...
    char at_rep[128] = {0};
    char cid_val[8] = {0};
    char sys_ram_val[32] = {0};

    sprintf(sys_ram_val, "cid_%d_state", cid_index);

    nv_get(sys_ram_val, cid_val, sizeof(cid_val));

    if(atoi(cid_val) != act_val)
    {
...
int telitParam_updateCgcontrdp ( int apn_index )
{
    ...
    ret = parsing_telit_cgcontrdp ( apn_index );
    return ret;
}
...
int telitParam_updateDualSim (void)
{
    ...
    COMD_AT_PROCESS ( "AT+CSUS?", 3000, at_rep, sizeof ( at_rep ) );
        ...
        if(strcmp ( cur_sim_mode, "0" ) == 0)
        {
            if(strcmp ( esim_supp, "1" ) == 0)
            {
                if(g_simslot_switch_rec == 0)
                {
                    ...
                    comd_wait_ms ( 1000 );
                    snprintf(at_req, sizeof ( at_req ), "AT+CSUS=1");
        ...
        else if(strcmp ( cur_sim_mode, "1" ) == 0)
        {
            if(strcmp ( esim_supp, "1" ) == 0)
            {
                if(g_simslot_switch_rec == 0)
                {
                    system("/lib/simslot_switch.sh sim");
                    comd_wait_ms ( 1000 );
                    snprintf(at_req, sizeof ( at_req ), "AT+CSUS=0");
                    g_simslot_switch_rec = 1;
                }
                else if(g_simslot_switch_rec == 1)
                {
...
static void telitParam_radioRestart (void)
{
    char at_rep[64] = {0};
...
static int telitParam_checkAbnormal ( char* sim_stat, char* main_stat )
{
    int ret = 1;
    char radio_stat[8] = {0};
...
void telit_normal_status_refresh (void)
{
    ...
    if ( 0 == strncmp ( dualsim_en, "1", 1 ) && telitParam_updateDualSim () && g_simslot_switch_event == 1)
    {
        g_simslot_switch_event = 0;
        comd_wait_ms ( 10 * 1000 );
        telitParam_initAPNsetting ( NULL );
        telitParam_radioLockInit ( "restore_fullband" );
        nv_set ( "imsi", "--" );
        nv_set ( "SIM_SPN", "--" );
        return;
    }
    ...
    telitParam_checkAbnormal ( cpin_value, mStatusVal );

    if ( 0 == strcmp ( cpin_value, "READY" ) )
    {
        telitParam_updateImsi ();
        telitParam_updateSimSPN ();
        telitParam_getSimlock ( NULL );
        telitParam_updateIccid ();
        telitParam_updatePhoneNumber ();
        telitParam_updatePDPreturnFormat();
        ...
        cur_cfun_state = parsing_telit_cfun_get ( at_rep );
        ...
        telitParam_updateCesq ();

        nv_get ( "cesq", cesq_value, sizeof ( cesq_value ) );

        if ( strcmp ( cesq_value, "" ) && strcmp ( cesq_value, "99,99,255,255,255,255,255,255,255" ) )
        {
            telitParam_updateCops ();
            telitParam_updateCgatt ();
            telitParam_updateCereg ();

            telitParam_updateC5greg ();
            teliParam_updateServingCell ( NULL );
            teliParam_updateNeighbourCell ( );
            telitParam_updateSignalBar ();
            ...
            if ( 0 == strcmp ( cgatt_value, "1" ) )
            {
                ...
                /* Use orig qmi system as Modem Manager, we can not use CGACT to decide cid_state */
                //telitParam_updateCgact ();
                ...
                for ( i = 1; i <= 4; i++ )
                {
                    ...
                    if ( 0 == strcmp ( apn_act_state, "1" ) )
                    {
                        ...
                        {
                            snprintf ( strRamOpt, sizeof ( strRamOpt ), "/usr/sbin/telit-CM.sh %d start", i );
                            CLOGD( FINE, "Execute [%s]\n", strRamOpt);
                            system(strRamOpt);
                            comd_wait_ms ( 1500 );

                            ret = telitParam_updateCgcontrdp ( i );
                            if(ret > 0)
                                apnActOrDeactMsgEvent ( i, 1 );
                        }
                        else
                        {
                            snprintf ( strRamOpt, sizeof ( strRamOpt ), "/usr/sbin/telit-CM.sh %d stop", i );
                            CLOGD( FINE, "Execute [%s]\n", strRamOpt);
                            system(strRamOpt);
                            comd_wait_ms ( 1500 );
                        }
...
int telit_module_param_init()
{
    ...
    if(
        telitParam_setATEcho ( NULL ) ||
        telitParam_setCfunMode ( "off" ) ||
        telitParam_updateModuleVersion ( NULL ) ||
        telitParam_updateImei ( NULL ) ||
        telitParam_updateSN ( NULL ) ||
        telitParam_updateDefaultGateway ( NULL ) ||
        telitParam_updateSuppBands ( NULL ) ||
        telitParam_disableSimslotHotplug ( ) ||
        telitParam_updatePinStatus() ||
        telitParam_updatePinLockStatus () ||
        telitParam_updatePinPukCount () ||
        telitParam_init_apn_netdev () ||
        telitParam_initAPNsetting ( NULL ) ||
        telitParam_initNetSelectMode ( NULL ) ||
        telitParam_initC5greg ( NULL ) ||
        telitParam_setCfunMode ( "on" ) ||
        telitParam_radioLockInit ( NULL )
        )
    {
        return -1;
    }
